...
const GetAllEmployeeReport = () => {
    ...
    async function gedData() {
      try {
        const response = await axios.get(
          `${APIURL}/employee/get-all-employee`
        );
        if (response.status === 200) {
          SetOrder(response.data.EmployeeList);
  ...
  const downloadReport = () => {
    doc.text("Employee Details Report", 30, 10);
    ...
    order.map((orders, index) => {
      ...
      row.push(index + 1);
      row.push(orders.Nic);
      row.push(orders.emp_name);
      row.push(orders.emp_address);
      row.push(orders.emp_department);
      row.push(orders.emp_reg_date);
      row.push(orders.gender_status);
      row.push(orders.emp_salary);
      array.push(row);
      return row;
    });
        ...
        <div id="content-wrapper" className="d-flex flex-column">
          ...
          <div id="content">
            ...
            <nav className="navbar navbar-expand topbar mb-4 static-top">
              <h1 className="h3 mb-2 text-gray-800">All Employee Details List</h1>
            ...
            <div className="container-fluid">
              <p className="mb-4">All Employee Details available in here.</p>
...
export default GetAllEmployeeReport;
